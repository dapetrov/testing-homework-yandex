# Настройка багов
При запуске с 'bug_id=0' тесты не выдают ошибок. 
Для проверки работы тестов заменить все 'bug_id=0' на 'bug_id='номер теста'
Например  замена с помощью CTRL + SHIFT + H (VS Code) (извините если это не удобно, я не придумал другое)
...
npx hermione gui
# Проблемы
1-3 тесты не получилось отловить
